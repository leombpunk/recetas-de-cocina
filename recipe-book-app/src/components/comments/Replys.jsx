        ...
        <>
          ...
          >
            {replys.map((reply, index) => (
              <div
                key={index}
                className={`flex flex-col gap-0 my-0 ml-14`} //border px-2 py-3 rounded-xl shadow-md shadow-black/25 bg-orange-300/70
              >
                {/* <Link
                  to={"#"}
                ...
                <div className='flex flex-row w-full items-center gap-2 pl-10'>
                  ...
                  >
                    {show ? (
                      ...
                      <>
                        <XMarkIcon className='h-5 w-5' /> Canceclar
                      </>
                    )}
                  </button>
                  {user ? (
                ...
                <Comment onNotLogin={null} handleClickDeleteComment={null} comment={reply} user={user} isAnswer={true} commentId={commentId} /> 
              </div>
            ))}
          </Transition>
          {/* <Answer commentId={commentId} comment={replyData} isHidden={show} /> */}
        </>
      ) : (
